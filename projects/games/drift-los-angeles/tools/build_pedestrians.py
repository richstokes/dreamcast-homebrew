...
def write_header(path: Path, lods: list[list[Part]]) -> None:
    write_rig_header(
        path, "DRIFT_LA_PEDESTRIAN_DATA_H", "tools/build_pedestrians.py",
        "DLA_PED_", PART_ENUM, "DLA_PED_SLOT_", SLOT_ENUM,
        {f"dla_ped_lod{lod}": parts for lod, parts in enumerate(lods)},
        [f"#define DLA_PED_ATLAS_REGION ({REGION / ATLAS:.6f}f)",
         f"#define DLA_PED_HEAD_TOP ({HEAD_TOP:.4f}f)"],
        "dla_pedestrian_lods", "[2]", [[f"dla_ped_lod{lod}"] for lod in range(len(lods))])
...
def write_preview(directory: Path, atlas: Image.Image, lods: list[list[Part]]) -> None:
    ...
    for column, (seed, look) in enumerate(variants):
        ...
        toppers = ("hair_short", "hair_long", "cap")
        skip = [part.name in toppers and (look.topper == 3 or toppers[look.topper] != part.name)
                for part in parts]
        offsets = [part_tint_and_offset(part, look)[1] for part in parts]
        write_obj(directory / f"variant{column}.obj", f"variant{column}", world, parts,
                  offsets, skip, x_offset=column * .9)

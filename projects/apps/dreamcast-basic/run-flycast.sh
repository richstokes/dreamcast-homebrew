...
# Port A remains a controller with its VMU. Port B is transiently configured
# as a Dreamcast keyboard, with the host keyboard forwarded to it, without
# changing the user's saved Flycast settings.
exec "${FLYCAST_BIN}" \
    -config "input:device2=5,input:maple_sdl_keyboard=1,config:Debug.SerialConsoleEnabled=yes" \
    "${PROJECT_DIR}/dreamcast-basic.elf"

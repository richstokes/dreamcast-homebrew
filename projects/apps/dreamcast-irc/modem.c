/* Dial-up setup adapted from the DCVMU client; uses the same KOS PPP patch. */
#include "modem.h"
#include <kos.h>
#include <kos/sem.h>
...
static int run_step(connect_job *job,const char *message) {
    int canceled=irc_modem_progress(message);
    ...
    while(sem_wait_timed(&job->done,100)<0)
        canceled|=irc_modem_progress(message);
...
static void load_settings(flashrom_ispcfg_t *cfg) {
    if(flashrom_get_pw_ispcfg(cfg)==0 && has_phone(cfg)) {
        printf("Dreamcast IRC: modem using saved PlanetWeb ISP profile\n");return;
    }
    if(flashrom_get_ispcfg(cfg)==0 && has_phone(cfg)) {
        printf("Dreamcast IRC: modem using saved DreamPassport ISP profile\n");return;
    }
    memset(cfg,0,sizeof(*cfg));
    strcpy(cfg->phone1,"555");
    printf("Dreamcast IRC: modem using DreamPi defaults (no saved dial profile)\n");
}
...
void irc_modem_shutdown(void) {
    /* Requires ../../dcvmu.com/client/patches/kos-ppp-lifecycle.patch: stop/join PPP before freeing
...
int irc_modem_connect(void) {
    ...
    const char *failure="Modem startup failed. Use /reconnect to retry.";
    ...
    if(result<0) {
        failure=result==-2?"No dial tone. Check DreamPi and the phone cable.":
                ...
                "Modem did not connect. Check DreamPi and use /reconnect.";
        goto failed;
    }
    printf("Dreamcast IRC: modem carrier %d bps; negotiating PPP\n",job.rate);
    job.step=2;
    ...
    if(!net_default_dev || strcmp(net_default_dev->name,"ppp") ||
       !(net_default_dev->ip_addr[0]|net_default_dev->ip_addr[1]|
        ...
        failure="PPP did not supply an IP address. Use /reconnect to retry.";goto failed;
    }
    ...
    printf("Dreamcast IRC: PPP ready, IP %u.%u.%u.%u DNS %u.%u.%u.%u\n",
           dev->ip_addr[0],dev->ip_addr[1],dev->ip_addr[2],dev->ip_addr[3],
           dev->dns[0],dev->dns[1],dev->dns[2],dev->dns[3]);
    return 0;
failed:
    irc_modem_shutdown();
    if(result==-10)failure="Modem connection canceled.";
    if(result==-11)failure="Could not start modem worker. Use /reconnect to retry.";
    irc_modem_status(failure);
    printf("Dreamcast IRC: %s (SDK result %d)\n",failure,result);
    return -1;
}
...
int irc_modem_online(void) {
    return ppp_owned && modem_is_connected() && net_default_dev &&

...
struct fix32
{
    ...
    // KOS/newlib defines int32_t as long, distinct from its 32-bit int.
    inline fix32(int x) : m_bits(int32_t(uint32_t(x) << 16)) {}
    inline explicit fix32(unsigned int x) : m_bits(int32_t(uint32_t(x) << 16)) {}
    inline explicit operator int() const { return int(m_bits >> 16); }
    inline explicit operator unsigned int() const { return unsigned(m_bits >> 16); }
#endif

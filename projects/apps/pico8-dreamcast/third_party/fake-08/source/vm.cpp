...
void printLuaStack(lua_State* L) {
    ...
    for (unsigned index = top; index > 0; index--)
    {
        ...
        switch (type)
        {
            ...
            // numbers
            case LUA_TNUMBER: {
                // KOS libstdc++ does not provide floating-point std::to_string.
                char number[48];
                snprintf(number, sizeof(number), "%.6f", double(lua_tonumber(L, index)));
                str = str + number + "\n";
                break;
            }
...
void Vm::CloseCart() {
    // Dreamcast launcher can replace a cart while its pause menu is open.
    _pauseMenu = false;
    _clearInputOnResume = true;
    _audio->setPaused(false);
    if (_loadedCart){

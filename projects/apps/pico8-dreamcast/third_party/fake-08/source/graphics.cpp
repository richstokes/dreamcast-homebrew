	...
	bool flip_y) 
{
    ...
    // Dreamcast: avoid per-row work for entirely clipped sprites.
    if (scr_w <= 0 || scr_h <= 0 || scr_x >= drawState.clip_xe ||
        scr_y >= drawState.clip_ye || scr_x + scr_w <= drawState.clip_xb ||
        scr_y + scr_h <= drawState.clip_yb) return;
...
void Graphics::map(int celx, int cely, int sx, int sy, int celw, int celh, uint8_t layer) {
    // Only visible tiles can affect the framebuffer. In particular, map()
    // defaults to 128x32 tiles while the screen shows just 16x16.
    const auto& ds = _memory->drawState;
    int screenX = sx - ds.camera_x, screenY = sy - ds.camera_y;
    int x0 = std::max(0, (int(ds.clip_xb) - screenX) / 8);
    int y0 = std::max(0, (int(ds.clip_yb) - screenY) / 8);
    int x1 = std::min(celw, (int(ds.clip_xe) - screenX + 7) / 8);
    int y1 = std::min(celh, (int(ds.clip_ye) - screenY + 7) / 8);
	for (int y = y0; y < y1; y++) {
		for (int x = x0; x < x1; x++) {
